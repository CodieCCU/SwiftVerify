...
const initializeSESClient = () => {
  ...
  if (!accessKeyId || !secretAccessKey) {
    console.warn('⚠️  AWS credentials not configured. Email sending will be simulated.');
    return null;
  }
 ...
 * @param {string} emailParams.htmlBody - HTML email body
...
export const sendEmail = async (emailParams) => {
  ...
  if (!client) {
    console.log('🔄 Simulating email send (no AWS credentials configured)');
    ...
    const params = {
      ...
      Message: {
        ...
        Body: {
          ...
          Text: {
            Data: textBody || htmlBody.replace(/<[^>]*>/g, ''), // Strip HTML if no text provided
            Charset: 'UTF-8',
          },
        },
      },
